...
from sph_dem.particle_shifting.rigid_fluid_coupling import (
...
class Problem(Application):
    ...
    def create_rb_geometry_particle_array(self):
        x, y = get_2d_block(self.dx, 8. * self.dx, 8. * self.dx)
        z = np.zeros_like(x)
        ...
        x_circle, y_circle = get_2d_block(self.dx, 8. * self.dx, 8. * self.dx)
    ...
    def create_particles(self):
        ...
        rigid_body_combined.y[:] += self.fluid_height / 4.
        ...
        # remove fluid particles overlapping with the rigid body
        G.remove_overlap_particles(
            fluid, rigid_body_slave, 0.8 * self.dx, dim=self.dim
        )
        ...
        # add a boundary like body in between
        ...
        moving_boundary.y[:] += self.fluid_height / 2.
        moving_boundary.y[:] += 4. * self.dx
        # G.remove_overlap_particles(
        #     fluid, moving_boundary, 1.5 * self.dx, dim=self.dim
        # )
