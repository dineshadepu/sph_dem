...
class Problem(Application):
    def add_user_options(self, group):
        group.add_argument("--fluid-length-ratio", action="store", type=float,
                           dest="fluid_length_ratio", default=6,
        ...
        add_bool_argument(
            ...
            dest='swelling',
            default=True,
            help='Mention if the particles should swell')
    ...
    def consume_user_options(self):
        ...
        self.max_rigid_body_diameter = 2. * 0.11
        ...
        self.swelling = self.options.swelling
        # ==========================
        # Numerical properties ends
        # ==========================
    ...
    def create_six_bodies(self):
        ...
        x2 += 1.5 * self.rigid_body_diameter + self.dx * 2
        x3[:] = x2[:] + 1.5 * self.rigid_body_diameter + self.dx * 4

        x4, y4 = create_circle_1(self.rigid_body_diameter, self.dx)
        x4[:] = x1[:] + 1. * self.rigid_body_diameter + self.dx * 0
        y4[:] = y4[:] + 1. * self.rigid_body_diameter + self.dx * 0
        x5, y5 = create_circle_1(self.rigid_body_diameter, self.dx)
        x5[:] = x2[:] + 1. * self.rigid_body_diameter + self.dx * 0
        y5[:] = y5[:] + 1. * self.rigid_body_diameter + self.dx * 0

        # x2 += x1 + self.rigid_body_diameter + self.dx * 2
        # x3 += x2 + self.rigid_body_diameter + self.dx * 2

        x_three_bottom = np.concatenate((x1, x2, x3, x4, x5))
        y_three_bottom = np.concatenate((y1, y2, y3, y4, y5))

        # Move all the bodies to the bottom
        y_three_bottom[:] -= self.rigid_body_diameter
    ...
    def create_particles(self):
        ...
        rigid_body_combined.x[:] += self.fluid_length / 2.
        ...
        rigid_body_combined.y[:] -= min(rigid_body_combined.y) - min(fluid.y)
        ...
        rigid_body_master, rigid_body_slave = get_master_and_slave_swelling_rb_from_combined(
            rigid_body_combined, self.max_rigid_body_diameter, self.dx)
        ...
        rigid_body_slave.m[:] = self.fluid_rho * self.dx**2. * 1 / (1)
    ...
    def create_equations(self):
        ...
        # eqns.groups[1].append(Group(equations=tmp, condition=swell_condition))
            eqns.groups[1].insert(1, Group(equations=tmp, condition=swell_condition))
