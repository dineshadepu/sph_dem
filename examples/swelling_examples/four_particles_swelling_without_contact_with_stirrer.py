...
class Problem(Application):
    ...
    def consume_user_options(self):
        ...
        self.T = (self.fluid_length - 8. * self.stirrer_length) / self.stirrer_velocity
        ...
        if self.swelling == True:
            ...
            for i in range(len(master.x)):
                if master.rad_s[i] < self.max_rigid_body_diameter / 2. - 2. * self.dx:
                    master.rad_s[i] += (self.dx / 8.) / readjust_freq
                    master.m[i] = np.pi * master.rad_s[i]**2. * self.rigid_body_rho

                if (int(t / dt) % readjust_freq) == 0:
                    # re-adjust the SPH particles
                    x_new, y_new = create_circle_of_three_layers(master.rad_s[i] * 2. - self.dx / 20.,
                                                                 self.dx)
