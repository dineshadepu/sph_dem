...
class Problem(Application):
    ...
    def consume_user_options(self):
        ...
        self.max_rigid_body_diameter = 3 * 0.11
    ...
    def post_step(self, solver):
        ...
        for i in range(len(master.x)):
            ...
            # master.c_wp[i] = (master.m_b[i] - master.m_p_0) / master.v_s_t

            master.rad_s[i] += self.dx / 8 * 1 / 700
            master.m_b[i] = np.pi * master.rad_s[i]**2. * self.rigid_body_rho
            master.radial_vel_count[i] += 1

            if master.rad_s[i] - master.rad_s_prev[i] >= self.dx / 8:
